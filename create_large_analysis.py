...
def main():
    ...
    for file in jsonl_files:
        ...
        if "fullText" in df.columns:
            df["cleaned_text"] = (
                df["fullText"].to_pandas().astype(str).apply(clean_text)
            )
    ...
    hdbscan_model = HDBSCAN(min_cluster_size=10, gen_min_span_tree=True, min_samples=3)
    ...
    topic_model.visualize_topics(top_n_topics=20).write_html(
